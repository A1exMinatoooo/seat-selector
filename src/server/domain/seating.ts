...
import { eventAuditLogs, eventSeats, events, participants, reservations, reservationSeats, seats } from "@/server/db/schema";
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      return await getDb().transaction(async (tx) => {
        const [event] = await tx.select({ status: events.status }).from(events).where(eq(events.id, input.eventId)).limit(1).for("update");
        ...
        const valid = await tx.select({ id: seats.id, rowLabel: seats.rowLabel, columnLabel: seats.columnLabel }).from(seats).innerJoin(eventSeats, and(eq(eventSeats.seatId, seats.id), eq(eventSeats.eventId, input.eventId))).where(and(eq(seats.hallId, input.hallId), eq(seats.kind, "seat"), eq(seats.selectable, true), inArray(seats.id, input.seatIds)));
