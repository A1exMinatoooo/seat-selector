...
import { eventAuditLogs, eventSeats, events, halls, participants, reservations, reservationSeats, seats } from "@/server/db/schema";
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      return await getDb().transaction(async (tx) => {
        const [event] = await tx.select({ status: events.status, lockedSeatHalf: events.lockedSeatHalf, centerAfterColumn: halls.centerAfterColumn }).from(events).innerJoin(halls, eq(events.hallId, halls.id)).where(eq(events.id, input.eventId)).limit(1).for("update");
        ...
        const [valid, hallSeats] = await Promise.all([
          tx.select({ id: seats.id, rowLabel: seats.rowLabel, columnLabel: seats.columnLabel }).from(seats).innerJoin(eventSeats, and(eq(eventSeats.seatId, seats.id), eq(eventSeats.eventId, input.eventId))).where(and(eq(seats.hallId, input.hallId), eq(seats.kind, "seat"), eq(seats.selectable, true), inArray(seats.id, input.seatIds))),
          tx.select({ id: seats.id, columnIndex: seats.columnIndex, kind: seats.kind, templateSelectable: seats.selectable }).from(seats).where(eq(seats.hallId, input.hallId)),
        ]);
        const effectiveRequested = effectiveEventAvailability(
          hallSeats,
          valid.map((seat) => seat.id),
          event.lockedSeatHalf,
          event.centerAfterColumn,
        );
        if (valid.length !== person.total || effectiveRequested.length !== person.total) throw new DomainError(errorCodes.validation, "Invalid seat", 400);
