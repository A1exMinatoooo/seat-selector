...
import { setEntryClaim, setParticipantClaim } from "@/server/security/participant-session";
...
export async function POST(request: Request) { try { assertSameOrigin(request); if (!rateLimit(`redeem:${clientAddress(request)}`, 30, 60_000)) return Response.json({ error: "RATE_LIMITED" }, { status: 429 }); const input = schema.parse(await request.json()); const [event] = await getDb().select({ id: events.id, status: events.status, participationMode: events.participationMode }).from(events).where(eq(events.publicCode, input.code)).limit(1); if (!event || event.status !== "open") throw new DomainError(errorCodes.forbidden, "活动已关闭，请联系现场工作人员", 403);
  if (event.participationMode === "onsite") {
    ...
    if (!existingToken) jar.set("ps_device", deviceToken, { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/", maxAge: 180 * 24 * 60 * 60 });
    await setParticipantClaim(claim);
  } else {
    await consumeQrToken(event.id, input.code, input.token);
    await setEntryClaim({ eventId: event.id, code: input.code });
  }
  return Response.json({ ok: true }); } catch (error) { return apiFailure(error); } }
