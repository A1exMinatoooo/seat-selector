...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const types = await getDb().select({ id: ticketTypes.id, name: ticketTypes.name, lotteryEligible: ticketTypes.lotteryEligible }).from(ticketTypes).where(eq(ticketTypes.eventId, id)).orderBy(asc(ticketTypes.sortOrder));
