...
export default async function ParticipantsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [people, allocations, types, reservationRows, seatRows] = await Promise.all([
    ...
    getDb()
      .select({
        participantId: participantTickets.participantId,
  ...
  return (
    <main className="admin-shell">
      ...
      <nav className="crumbs">
        ...
        <span>/</span>
        <strong>{event.participationMode === "onsite" ? "发行记录" : "参与者"}</strong>
      </nav>
      <header className="section-header">
        <div>
          <p className="eyebrow">
            {event.participationMode === "onsite" ? "现场领取" : "参与清单"}
          </p>
          <h1>
            {people.length} {event.participationMode === "onsite" ? "笔发行记录" : "位参与者"}
          </h1>
        </div>
        <a className="button" href={`/api/admin/events/${id}/export.csv`}>
          导出选座记录
        </a>
      </header>

      {event.participationMode === "preregistered" ? (
        <div className="admin-grid participant-entry-grid">
          <section className="panel">
            <h2>CSV 批量导入</h2>
            <p className="muted">
              模板会按当前票种生成列：昵称、手机号或尾号、
              {types
                .map((type) => `${type.name}${type.lotteryEligible ? "（参与抽奖）" : ""}`)
                .join("、")}
              。昵称请填写社交平台昵称，不要填写真实姓名。
            </p>
            <div className="header-actions">
              <a className="button" href={`/api/admin/events/${id}/participants/template.csv`}>
                下载导入模板
              </a>
            </div>
            {editable ? (
              <AdminActionForm
                ...
                className="stack-form participant-import-form"
                resetOnSuccess
              >
          ...
          <section className="panel">
            <h2>手动增加参与者</h2>
            <p className="muted">
              适合临时补录单个参与者；只存储社交平台昵称，不存储参与者真实姓名。手机号可填写完整号码或四位尾号。
            </p>
            {editable ? (
              <AdminActionForm action={addParticipantAction} className="stack-form" resetOnSuccess>
                <input type="hidden" name="eventId" value={id} />
                ...
                <AdminSubmitButton pendingLabel="正在增加…">增加参与者</AdminSubmitButton>
              </AdminActionForm>
            ) : (
        ...
        {people.length ? (
          <div className="table-wrap">
            <table>
              <thead>
                <tr>
                  <th>{event.participationMode === "onsite" ? "编号" : "昵称"}</th>
                  {event.participationMode === "preregistered" ? <th>手机</th> : null}
                  <th>票种</th>
                  <th>座位</th>
                  <th>选座确认时间</th>
                  <th>设备</th>
                  ...
                  <tr key={person.id}>
                    ...
                    <td>
                      {(byPerson.get(person.id) ?? [])
                        .map((ticket) => `${ticket.name} × ${ticket.quantity}`)
                        .join("、")}
                    </td>
                    <td>
                    ...
                    <td>
                      {confirmationTime(reservationMap.get(person.id)?.confirmedAt, event.timeZone)}
                    </td>
                    <td>{person.deviceBoundAt ? "已绑定" : "未绑定"}</td>
                    <td>
                      <div className="row-actions">
                        {person.deviceBoundAt ? (
                          <AdminActionForm action={resetDeviceAction}>
                            <input type="hidden" name="eventId" value={id} />
                            <input type="hidden" name="participantId" value={person.id} />
                            <AdminSubmitButton className="text-button" pendingLabel="解绑中…">
                              解绑设备
                            </AdminSubmitButton>
                          </AdminActionForm>
                        ) : null}
                        <AdminActionForm action={toggleLocationExemptionAction}>
                          <input type="hidden" name="eventId" value={id} />
                          <input type="hidden" name="participantId" value={person.id} />
                          <input
                            type="hidden"
                            name="enabled"
                            value={person.locationExemptAt ? "0" : "1"}
                          />
                          <AdminSubmitButton className="text-button" pendingLabel="更新中…">
                            {person.locationExemptAt ? "取消定位豁免" : "定位豁免"}
                          </AdminSubmitButton>
                        </AdminActionForm>
