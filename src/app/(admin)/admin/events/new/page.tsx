import Link from "next/link";
import { asc, eq } from "drizzle-orm";
import { TicketTypeFields } from "@/features/events/ticket-type-fields";
import { getDb } from "@/server/db/client";
import { cinemas, halls, locationPresets } from "@/server/db/schema";
import { requireAdmin } from "@/server/security/admin-session";
import { createEventAction } from "../actions";

export const dynamic = "force-dynamic";

export default async function NewEventPage() {
  await requireAdmin();
  const [hallRows, locations] = await Promise.all([
    getDb().select({ id: halls.id, hall: halls.name, cinema: cinemas.name }).from(halls).innerJoin(cinemas, eq(halls.cinemaId, cinemas.id)).orderBy(asc(cinemas.name), asc(halls.name)),
    getDb().select().from(locationPresets).orderBy(asc(locationPresets.name)),
  ]);
  return <main className="admin-shell"><nav className="crumbs"><Link href="/admin/events">活动</Link><span>/</span><strong>新建</strong></nav><header className="section-header"><div><p className="eyebrow">新活动</p><h1>建立选座活动</h1></div></header>{hallRows.length && locations.length ? <form action={createEventAction} className="panel stack-form"><div className="form-row"><label>活动名称<input name="name" required placeholder="例如：八月特别观影会" /></label><label>开始时间<input name="startsAt" type="datetime-local" required /></label></div><div className="form-row"><label>影厅<select name="hallId">{hallRows.map((hall) => <option key={hall.id} value={hall.id}>{hall.cinema} · {hall.hall}</option>)}</select></label><label>活动地点<select name="locationId">{locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</select></label></div><div className="form-row"><label>定位半径（米）<input name="radiusMeters" type="number" min="50" defaultValue="1000" /></label><label>显示时区<input name="timeZone" defaultValue="Asia/Shanghai" /></label></div><TicketTypeFields /><button className="button primary" type="submit">保存草稿</button></form> : <section className="panel"><p>请先建立至少一个影厅模板和活动地点。</p></section>}</main>;
}
