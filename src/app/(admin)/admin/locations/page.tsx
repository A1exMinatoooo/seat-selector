...
export default async function LocationsPage() {
  ...
  return <main className="admin-shell">
    ...
    <div className="admin-grid">
      ...
      <section className="panel"><h2>地点列表</h2>{locations.length ? <ul className="record-list actionable-record-list">{locations.map((location) => <li key={location.id}><div><strong>{location.name}</strong><small>{location.latitude.toFixed(5)}, {location.longitude.toFixed(5)}</small></div><div className="row-actions"><span>{location.defaultRadiusMeters} m</span><Link className="button" href={`/admin/locations/${location.id}/edit`} aria-label={`编辑地点 ${location.name}`}>编辑</Link><LocationDeleteButton id={location.id} label={location.name} /></div></li>)}</ul> : <p className="muted">还没有预录地点。</p>}</section>
    </div>
  </main>;
}
