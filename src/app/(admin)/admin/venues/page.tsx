...
export default async function VenuesPage() {
  ...
  const cinemaSummaries = cinemaRows.flatMap((cinema) => {
    ...
    return cinemaHalls.length ? [{ id: cinema.id, name: cinema.name, hallCount: cinemaHalls.length }] : [];
  });
  const templateGroups = cinemaSummaries.map((cinema) => ({
    ...cinema,
    content: <ul className="record-list actionable-record-list">{hallRows.filter((hall) => hall.cinemaId === cinema.id).map((hall) => {
