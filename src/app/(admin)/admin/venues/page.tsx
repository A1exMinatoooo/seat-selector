...
export default async function VenuesPage() {
  ...
  const [cinemaRows, hallRows] = await Promise.all([
    ...
    getDb().select({ id: halls.id, name: halls.name, cinemaName: cinemas.name, seatCount: sql<number>`count(${seats.id})` }).from(halls).innerJoin(cinemas, eq(halls.cinemaId, cinemas.id)).leftJoin(seats, eq(seats.hallId, halls.id)).groupBy(halls.id, cinemas.name).orderBy(asc(cinemas.name), asc(halls.name)),
  ]);
  return (
    <main className="admin-shell">
      ...
      <div className="admin-grid">
        ...
        <section className="panel"><h2>已有模板</h2>{hallRows.length ? <ul className="record-list">{hallRows.map((hall) => <li key={hall.id}><strong>{hall.cinemaName} · {hall.name}</strong><span>{hall.seatCount} 个网格单元</span></li>)}</ul> : <p className="muted">还没有影厅模板。</p>}</section>
      </div>
