  ...
  const [cinemaRows, hallRows] = await Promise.all([
    ...
    getDb()
      .select({
        ...
        seatCount: sql<number>`count(${seats.id})`,
      })
      .from(halls)
      .innerJoin(cinemas, eq(halls.cinemaId, cinemas.id))
      .leftJoin(seats, eq(seats.hallId, halls.id))
      .where(isNull(halls.archivedAt))
      .groupBy(halls.id, cinemas.name)
      .orderBy(asc(cinemas.name), asc(halls.name)),
  ]);
  const [eventRows, seatRows] = hallRows.length
    ? await Promise.all([
        getDb()
    ...
    : [[], []];
  const cinemaSummaries = cinemaRows.flatMap((cinema) => {
    const cinemaHalls = hallRows.filter((hall) => hall.cinemaId === cinema.id);
    return cinemaHalls.length
      ? [{ id: cinema.id, name: cinema.name, hallCount: cinemaHalls.length }]
      : [];
  });
  const templateGroups = cinemaSummaries.map((cinema) => ({
    ...cinema,
    content: (
      <ul className="record-list actionable-record-list">
        {hallRows
          .filter((hall) => hall.cinemaId === cinema.id)
          .map((hall) => {
            const linkedStatuses = eventRows
              .filter((event) => event.hallId === hall.id)
              .map((event) => event.status);
            const editable = canEditHallTemplate(linkedStatuses);
            const label = `${hall.cinemaName} · ${hall.name}`;
            return (
              <li key={hall.id}>
                <div>
                  <strong>{hall.name}</strong>
                  <span>{hall.seatCount} 个网格单元</span>
                </div>
                <div className="row-actions">
                  <HallLayoutPreviewDialog
                    label={label}
                    cells={seatRows.filter((seat) => seat.hallId === hall.id)}
                    centerAfterColumn={hall.centerAfterColumn}
                  />
                  <a href={`/api/admin/venues/export?scope=hall&id=${hall.id}`}>导出</a>
                  {editable ? (
                    <Link className="text-button" href={`/admin/venues/${hall.id}/edit`}>
                      编辑
                    </Link>
                  ) : (
  ...
  return (
    <main className="admin-shell">
      ...
      <header className="section-header">
        <div>
          <p className="eyebrow">场地基础资料</p>
          <h1>影院与影厅</h1>
        </div>
        <span>{hallRows.length} 个影厅模板</span>
      </header>
      <div className="admin-grid">
        ...
        <section className="panel">
          <div className="section-header compact">
            <h2>已有模板</h2>
            {hallRows.length ? <HallTemplateExportMenu cinemas={cinemaSummaries} /> : null}
          </div>
          {hallRows.length ? (
            <HallTemplateGroups groups={templateGroups} />
          ) : (
            <p className="muted">还没有影厅模板。</p>
          )}
        </section>
      </div>
      <section className="panel wide">
        <h2>导入影厅模板</h2>
        <p className="muted">
          支持导入单个影厅、单个影院或全部影院导出的 JSON
          文件。同名影院会复用，影厅模板会新增，不覆盖已有模板。
        </p>
        <HallTemplateImportForm />
      </section>
      <section className="panel wide">
        <h2>新建影厅模板</h2>
        {cinemaRows.length ? (
