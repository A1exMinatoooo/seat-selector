  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        if (
          ending &&
          !window.confirm("确定要结束该活动吗？结束后将不能继续选座或修改座位开放范围。")
        )
          event.preventDefault();
        if (
          reopening &&
          !window.confirm(
            "确定要重新开放该活动吗？参与者将可以再次进入活动，现有名单与选座记录会保留。",
          )
        )
          event.preventDefault();
      }}
    >
      ...
      <button
        className={`button ${ending ? "danger" : "primary"}`}
        type="submit"
        disabled={pending}
      >
        {pending ? "正在更新…" : ending ? "结束活动" : reopening ? "重新开放" : "开放选座"}
      </button>
    </form>
  );
}
