...
const eventConfigurationFields = {
  ...
  locationCheckEnabled: z.preprocess(
    (value) => value === true || value === "on" || value === "true",
    z.boolean(),
  ),
  lotteryEnabled: z.preprocess(
    (value) => value === true || value === "on" || value === "true",
    z.boolean(),
  ),
  participationMode: z.enum(["onsite", "preregistered"]).default("onsite"),
  maxTicketsPerIssue: z.coerce.number().int().min(1).max(20).default(7),
...
function validateEventConfiguration(
  input: z.infer<z.ZodObject<typeof eventConfigurationFields>>,
...
export const eventConfigurationInputSchema = z
  .object({
    id: z.string().uuid(),
