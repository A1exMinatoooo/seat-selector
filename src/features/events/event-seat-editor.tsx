...
export function EventSeatEditor({
  ...
  initialLockedSeatHalf?: SeatHalf | null;
  ...
  const hallGroups = useMemo(() => {
    const groups: { id: string; label: string; options: { id: string; label: string }[] }[] = [];
    for (const item of halls) {
      const group = groups.find((candidate) => candidate.id === item.cinemaId);
      const option = { id: item.id, label: item.hallName };
      if (group) group.options.push(option);
      else groups.push({ id: item.cinemaId, label: item.cinemaName, options: [option] });
    }
    return groups;
  }, [halls]);
  ...
  const [changeSource, setChangeSource] = useState<
    "manual" | "half_lock" | "half_unlock" | "half_switch" | "quick_count" | "rectangle_toggle"
  >("manual");
  const [changedSide, setChangedSide] = useState<SeatHalf | "">("");
  const [lockedSeatHalf, setLockedSeatHalf] = useState<SeatHalf | null>(initialLockedSeatHalf);
  ...
  const effectiveAvailableSeatIds = effectiveEventAvailability(
    positionedSeats,
    available,
    lockedSeatHalf,
    centerAfterColumn,
  ...
  function toggleHalf(side: SeatHalf) {
    const result = toggleSeatHalfLock(lockedSeatHalf, side);
    setLockedSeatHalf(result.activeSide);
  ...
  return (
    <fieldset className="layout-editor event-seat-editor">
      ...
      {includeHallSelect ? (
        <div className="event-hall-field">
          <SelectField
            ...
            groups={hallGroups}
            value={hall.id}
            displayValue={(nextHallId) => {
              const selected = halls.find((item) => item.id === nextHallId);
              return selected ? `${selected.cinemaName} · ${selected.hallName}` : "请选择影厅";
            }}
      ...
      {enableHalfLockControls ? (
        <div className="half-lock-controls">
          <div>
            ...
            <p className="muted">
              半场锁定只过滤基础开放范围；再次点击当前启用的按钮可取消锁定。锁定期间仍可调整基础开放范围，保存后统一生效。
            </p>
          </div>
          <div className="header-actions">
            <button
              className={`button ${lockedSeatHalf === "left" ? "primary" : ""}`}
              aria-pressed={lockedSeatHalf === "left"}
              type="button"
              onClick={() => toggleHalf("left")}
            >
              {lockedSeatHalf === "left" ? "取消锁定左半场" : "锁定左半场"}
            </button>
            <button
              className={`button ${lockedSeatHalf === "right" ? "primary" : ""}`}
              aria-pressed={lockedSeatHalf === "right"}
              type="button"
              onClick={() => toggleHalf("right")}
            >
              {lockedSeatHalf === "right" ? "取消锁定右半场" : "锁定右半场"}
            </button>
          </div>
        </div>
      ) : null}
              ...
              {hall.seats
                ...
                .map((seat) => {
                  ...
                  const isHalfLocked =
                    ...
                    !isLocked &&
                    isSeatLockedByHalf(
                      { ...seat, templateSelectable: seat.selectable },
                      lockedSeatHalf,
                      centerAfterColumn,
                      positionedSeats,
                    );
      ...
      <input type="hidden" name="lockedSeatHalf" value={lockedSeatHalf ?? ""} />
