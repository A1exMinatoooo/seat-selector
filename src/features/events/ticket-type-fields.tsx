...
export function TicketTypeFields({
  ...
  const [prizes, setPrizes] = useState<PrizeValue[]>(() =>
    (initialPrizes.length ? initialPrizes : [{ name: "", quantity: 1 }]).map((prize, index) => ({
      ...prize,
      key: `prize-initial-${index}`,
    })),
  );
      ...
      <fieldset className="participation-mode-fields">
        ...
        <div className="segmented-control" role="radiogroup" aria-label="活动参与方式">
          <label>
            <input
              ...
              value="onsite"
              checked={participationMode === "onsite"}
              onChange={() => setParticipationMode("onsite")}
            />
            <span>现场发行</span>
          </label>
          <label>
            <input
              ...
              name="participationMode"
              value="preregistered"
              checked={participationMode === "preregistered"}
              onChange={() => setParticipationMode("preregistered")}
            />
            <span>预录参与者</span>
          </label>
        </div>
        <p className="muted">
          {participationMode === "onsite"
            ? "现场选择票种和张数后发行单次二维码，无需预录昵称和手机号。"
            : "提前录入参与者的社交平台昵称、手机号及其购买票数，扫码后验证身份。请勿录入真实姓名。"}
        </p>
        {participationMode === "onsite" ? (
          <label>
            单次最多发行张数
            <NumericInput
              name="maxTicketsPerIssue"
              min={1}
              max={20}
      ...
      <fieldset className="lottery-fields">
        ...
        <label className="switch-label">
          <input
            ...
            onChange={(event) => {
              ...
              setTypes((current) => current.map((type) => ({ ...type, lotteryEligible: enabled })));
            }}
          />
        ...
        {lotteryEnabled ? (
          <>
            <p className="muted">
              {participationMode === "onsite"
                ? "总奖池人数 = 预计可抽奖票数 + 额外奖池人数 X；预计额度在扫码领取时占用。"
                : "开放选座前，请先在“参与者清单”录入参与者及其票数。总奖池人数 = “参与抽奖”的票数 + 额外奖池人数 X。"}{" "}
              奖品总数必须小于等于总奖池人数，其余为“未中奖”。
            </p>
            {participationMode === "onsite" ? (
              <label>
                预计可抽奖票数
                <NumericInput
                  name="expectedLotteryTickets"
                  min={1}
                  max={100000}
                  defaultValue={initialExpectedLotteryTickets ?? undefined}
                />
                <span className="muted">所有已领取的参与抽奖票累计不得超过此数量。</span>
              </label>
            ) : (
              <input type="hidden" name="expectedLotteryTickets" value="" />
            )}
            <label>
              额外奖池人数 X
              <NumericInput
                name="lotteryPoolBonus"
                min={0}
                max={100000}
                defaultValue={initialLotteryPoolBonus}
              />
              <span className="muted">总奖池人数 = 参与抽奖票数 + 额外奖池人数 X</span>
            </label>
                ...
                <div key={prize.key}>
                  ...
                  <NumericInput
                    ...
                    onValueChange={(quantity) =>
                      setPrizes((current) =>
                        current.map((item) =>
                          item.key === prize.key ? { ...item, quantity } : item,
                        ),
                      )
                    }
                  />
                  <button
                    ...
                    onClick={() =>
                      setPrizes((current) => current.filter((item) => item.key !== prize.key))
                    }
                  >
                    移除
                  </button>
            ...
            <button
              ...
              onClick={() =>
                setPrizes((current) => [
                  ...current,
                  { key: createKey("prize"), name: "", quantity: 1 },
                ])
              }
            >
              添加奖品
      ...
      <fieldset className="ticket-types">
        ...
        {types.map((type, index) => (
          <div className="ticket-type-row" key={type.key}>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setTypes((current) =>
                      current.map((item) =>
                        item.key === type.key
                          ? { ...item, lotteryEligible: event.target.checked }
                          : item,
                      ),
                    )
                  }
            ...
            <button
              ...
              onClick={() => setTypes((current) => current.filter((item) => item.key !== type.key))}
            >
              移除
            </button>
          </div>
        ...
        <button
          ...
          onClick={() =>
            setTypes((current) => [
              ...current,
              { key: createKey("ticket"), name: "", lotteryEligible: lotteryEnabled },
            ])
          }
        >
          添加票种
