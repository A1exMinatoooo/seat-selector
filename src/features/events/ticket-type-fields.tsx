...
export function TicketTypeFields() {
  ...
  const [prizes, setPrizes] = useState([{ id: 1, name: "", quantity: 1 }]);
  const nextPrizeId = useRef(2);
  return <>
    ...
    <fieldset className="lottery-fields"><legend>抽奖设置</legend><label className="check-label lottery-toggle"><input name="lotteryEnabled" type="checkbox" checked={lotteryEnabled} onChange={(event) => setLotteryEnabled(event.target.checked)} />此活动有抽奖</label>{lotteryEnabled ? <><p className="muted">只需录入实际奖品，系统会按参与抽奖的票数自动补足“未中奖”。</p><div className="prize-list">{prizes.map((prize, index) => <div key={prize.id}><input aria-label={`奖品名 ${index + 1}`} placeholder="奖品名" required value={prize.name} onChange={(event) => setPrizes((old) => old.map((item) => item.id === prize.id ? { ...item, name: event.target.value } : item))} /><NumericInput aria-label={`奖品数量 ${index + 1}`} min={1} max={100000} value={prize.quantity} onValueChange={(quantity) => setPrizes((old) => old.map((item) => item.id === prize.id ? { ...item, quantity } : item))} /><button type="button" disabled={prizes.length === 1} onClick={() => setPrizes((old) => old.filter((item) => item.id !== prize.id))}>移除</button></div>)}</div><button className="button" type="button" onClick={() => { const id = nextPrizeId.current; nextPrizeId.current += 1; setPrizes((old) => [...old, { id, name: "", quantity: 1 }]); }}>添加奖品</button></> : null}<input type="hidden" name="prizes" value={JSON.stringify(lotteryEnabled ? prizes.map(({ name, quantity }) => ({ name, quantity })) : [])} /></fieldset>
  </>;
}
